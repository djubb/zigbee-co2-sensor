...
- **Мониторинг батареи** — делитель B+ → 1МОм → GPIO1 → 1МОм → GND, читает ~3954 mV корректно
- **Веб-прошивалка** — работает, деплоится автоматически при push в main

### Известный баг

- `ESP_ERR_NOT_SUPPORTED` при попытке отправить unsolicited report по battery voltage (0x0020)
  - Атрибут не имеет флага `ESP_ZB_ZCL_ATTR_ACCESS_REPORTING` → ZHA не получает push-уведомления
  - Значение обновляется в памяти (через memcpy), ZHA может опросить (poll), но не получает авто-репорты
  - Требует правки в [main/zigbee.c](main/zigbee.c) — добавить флаг REPORTING к атрибуту
...
Читает ~3954 mV, показывает ~77% в ZHA. **Работает.**
